...
MQTTLIB_C_A = libpaho-mqtt3c.a
MQTTLIB_CS_A = libpaho-mqtt3cs.a
MQTTLIB_A_A = libpaho-mqtt3a.a
MQTTLIB_AS_A = libpaho-mqtt3as.a
...
MY_CCFLAGS_SO = -g -c  $(CFLAGS) -Os -Wall -fvisibility=hidden -I$(blddir_work)
...
MY_LDFLAGS_C = $(LDFLAGS) -Wl,-init,$(MQTTCLIENT_INIT) -lpthread
LDFLAGS_CS = $(LDFLAGS) -shared $(START_GROUP) -lpthread $(EXTRA_LIB) -lssl -lcrypto $(END_GROUP) -Wl,-init,$(MQTTCLIENT_INIT)
MY_LDFLAGS_CS = $(LDFLAGS) $(START_GROUP) -lpthread $(EXTRA_LIB) -lssl -lcrypto $(END_GROUP) -Wl,-init,$(MQTTCLIENT_INIT)
LDFLAGS_A = $(LDFLAGS) -shared -Wl,-init,$(MQTTASYNC_INIT) -lpthread
MY_LDFLAGS_A = $(LDFLAGS) -Wl,-init,$(MQTTASYNC_INIT) -lpthread
LDFLAGS_AS = $(LDFLAGS) -shared $(START_GROUP) -lpthread $(EXTRA_LIB) -lssl -lcrypto $(END_GROUP) -Wl,-init,$(MQTTASYNC_INIT)
MY_LDFLAGS_AS = $(LDFLAGS) $(START_GROUP) -lpthread $(EXTRA_LIB) -lssl -lcrypto $(END_GROUP) -Wl,-init,$(MQTTASYNC_INIT)
...
MY_CCFLAGS_SO += -Wno-deprecated-declarations -DOSX
...
${MQTTLIB_C_TARGET}: ${SOURCE_FILES_C} ${HEADERS_C} $(blddir_work)/VersionInfo.h
	${CC} ${MY_CCFLAGS_SO} ${SOURCE_FILES_C} ${MY_LDFLAGS_C}
	if [ ! -d source_o ]; then mkdir source_o; fi
	mv *.o source_o
	ar rs -o ${MQTTLIB_C_A} source_o/*.o
	ranlib ${MQTTLIB_C_A}
	if [ -d ./source_o ]; then rm -rf source_o; fi
...
${MQTTLIB_CS_TARGET}: ${SOURCE_FILES_CS} ${HEADERS_C} $(blddir_work)/VersionInfo.h
	${CC} ${MY_CCFLAGS_SO} ${SOURCE_FILES_CS} -DOPENSSL ${MY_LDFLAGS_CS}
	if [ ! -d source_o ]; then mkdir source_o; fi
	mv *.o source_o
	ar rs -o ${MQTTLIB_CS_A} source_o/*.o
	ranlib ${MQTTLIB_CS_A}
	if [ -d ./source_o ]; then rm -rf source_o; fi
...
${MQTTLIB_A_TARGET}: ${SOURCE_FILES_A} ${HEADERS_A} $(blddir_work)/VersionInfo.h
	${CC} ${MY_CCFLAGS_SO} ${SOURCE_FILES_A} ${MY_LDFLAGS_A}
	if [ ! -d source_o ]; then mkdir source_o; fi
	mv *.o source_o
	ar rs -o ${MQTTLIB_A_A} source_o/*.o
	ranlib ${MQTTLIB_A_A}
	if [ -d ./source_o ]; then rm -rf source_o; fi
...
${MQTTLIB_AS_TARGET}: ${SOURCE_FILES_AS} ${HEADERS_A} $(blddir_work)/VersionInfo.h
	${CC} ${MY_CCFLAGS_SO} ${SOURCE_FILES_AS} -DOPENSSL ${MY_LDFLAGS_AS}
	if [ ! -d source_o ]; then mkdir source_o; fi
	mv *.o source_o
	ar rs -o ${MQTTLIB_AS_A} source_o/*.o
	ranlib ${MQTTLIB_AS_A}
	if [ -d ./source_o ]; then rm -rf source_o; fi
